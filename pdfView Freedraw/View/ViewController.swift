...
//  pdfView Freedraw
//
//  Created by Bilal Ahmed on 02/01/2024.
//
...
class ViewController: UIViewController, UIGestureRecognizerDelegate, PDFFreedrawGestureRecognizerDelegate {
    ...
    @IBOutlet weak var btnRectangel: UIButton!
    ...
    func updateButtonsState() {
        if !pdfView.isUserInteractionEnabled { // Show controls when pdfView has no user interaction
            ...
            case .highlighter:
                ...
                if pdfFreedraw.convertClosedCurvesToOvalsRec {
                    btnRectangel.isSelected = true
                } else {
                    btnRectangel.isSelected = false
                }
                perfectOvalsOutlet.isEnabled = true
                btnRectangel.isEnabled = true
...
//        pdfFreedraw.convertClosedCurvesToOvals.toggle()
//        pdfFreedraw.convertClosedCurvesToOvalsRec.toggle()
        pdfFreedraw.convertClosedCurvesToOvals.toggle()
        pdfFreedraw.convertClosedCurvesToOvalsRec = false
        
        updateButtonsState()
    }
    
    @IBAction func Rectangel(_ sender: Any) {
        pdfFreedraw.convertClosedCurvesToOvalsRec.toggle()
        pdfFreedraw.convertClosedCurvesToOvals = false
        updateButtonsState()
    }
}
