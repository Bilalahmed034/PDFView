...
public class PDFFreedrawGestureRecognizer: UIGestureRecognizer {
    ...
    public var convertClosedCurvesToOvalsRec = false
        ...
        if let touch = touches.filter({String(format: "%p", $0)==self.firstTouchDetected}).first {
            DispatchQueue.main.async {
                ...
                if self.inkType == .eraser {
                    ...
                    if self.convertClosedCurvesToOvalsRec, self.inkType != .eraser {
                        let ovalBoundingBox = self.signingPath.bounds
                        self.signingPath = UIBezierPath(rect: ovalBoundingBox)
                    }
